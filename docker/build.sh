...
function build_substreams_image() {
    ...
    docker build --target devel --build-arg BASE_IMAGE="$stage_3_image" --build-arg PYTHON_VERSION="$python_version" \
...
BUF_VERSION="1.47.2"
SUBSTREAMS_VERSION="1.11.3"

dos2unix ./*
build_substreams_image "$PYTHON_VERSION" "$CMAKE_VERSION" "$BUF_VERSION" "$SUBSTREAMS_VERSION"